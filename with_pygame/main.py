...
class MyApp(object):
	...
	def _init_variables(self):
		...
		for i in range(len(self.video_infos_surface)):
			self.downloads.append(self.ft_gui.add_button(id_="button" + str(i), pos=(300 + i * 400, 600),
		...
		if ret["input_textbar"][self.textbar_keyword_yt.id_]:
			...
			for i in range(limit):
				if i < len(self.videos_infos["video_thumbnail_url"]):
					...
					self.downloads[i].display_on = True
				else:
					self.video_infos_surface[i] = False
					self.downloads[i].display_on = False

		for i in range(self.downloads_button):
			if ret["button"][self.downloads_button[i].id_]:
				file_video_name, file_audio_name = download_best_video_youtube(videos_infos["video_url"][i], download_path="video", callback=self._pafy_download_callback)
