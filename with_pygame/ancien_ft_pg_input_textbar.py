...
class FtPgInputTextbar(object):
	def __init__(
		...
		print("font_max_height", self.font_max_height)
		...
		self.cursor_start_visible = 0
		self.text = ""
		# distance top text never change
		# if text isnt centered distance left text doesn't change

		self.cursor_pos = len(self.text)
		self.cursor_pix_pos = 0
		# pour savoir commencer afficher le texte
		# de base tout est affiché donc collé a gauche
	...
	def get_actual_surface(self):
		...
		if self.active:
			...
			"""
			if left_text_surface.get_size()[0] > self.cursor_pix_pos:
				text_surface.blit(left_text_surface, (0 - (left_text_surface.get_size()[0] - self.cursor_pix_pos), 0))
			else:
				text_surface.blit(left_text_surface, (0, 0))

			#if right_text_surface.get_size()[0] > self.text_max_width - self.cursor_pix_pos:
			text_surface.blit(right_text_surface, (self.cursor_pix_pos + 2, 0))
			"""
			...
			#self.actual_surface.blit(self.cursor_surface, (self.distance_left_text + self.cursor_pix_pos, self.distance_top_text))
	...
	def display(self):
		...
		if self.display_on:
			...
			if self.active:
				...
				# get cursor position
				cursor_x_pos = self.font.size(self.text[:self.cursor_pos])[0]

				# si le curseur depasse a droite, on deplace tout vers la gauche
				if cursor_x_pos > self.text_max_width:
					x = cursor_x_pos - self.text_max_width


				# si le curseur depasse a gauche, on deplace tout vers la droite (au max 0)
				# on test sans pour l'instant
				###### a remplir

				# text_surface
				surface_tmp = self.font.render(self.text, self.text_antialias, self.text_color)
				text_surface = pg.Surface((self.text_max_width, surface_tmp.get_size()[1]))
				text_surface.fill(self.bg_color)
				text_surface.blit(surface_tmp, (0 - x, 0))

				surface.blit(text_surface, (self.distance_left_text, self.distance_top_text))
				surface.blit(self.cursor_surface,
					(self.distance_left_text + cursor_x_pos - x, self.distance_top_text))

			elif self.text != "":
				text_surface = self.font.render(self.text, self.text_antialias, self.text_color)

				surface.blit(text_surface, (self.distance_left_text, self.distance_left_text))

			elif self.placeholder:
				surface.blit(self.placeholder, (self.distance_left_text, self.distance_left_text))
